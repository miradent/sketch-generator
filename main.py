...
class SketchGenerator:
    ...
    def __init__(self, root):
        ...
        if getattr(sys, 'frozen', False):
            ...
            os.environ['RAPIDOCR_CONFIG_PATH'] = os.path.join(
                base_dir, 'rapidocr', 'default_models.yaml'
            )
            det_model_dir = os.path.join(base_dir, 'models', 'cnstd')
            rec_model_dir = os.path.join(base_dir, 'models', 'cnocr')
        else:
            det_model_dir = r"E:\BOK\论文\ai模型\cnstd"
            rec_model_dir = r"E:\BOK\论文\ai模型\cnocr"

        self.ocr_reader = CnOcr(
            det_model_name='ch_PP-OCRv3_det',
            rec_model_name='densenet_lite_136-gru',
            det_model_root_dir=det_model_dir,
            rec_model_root_dir=rec_model_dir
        )
